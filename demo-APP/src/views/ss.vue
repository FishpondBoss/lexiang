<template>
  <div id="app">
    <div class="top">
      <span class="fh" @click="black">×</span>
      <mt-button class="fb" @click="fb">发布</mt-button>
    </div>
    <div id="shang">
        <textarea v-model="msg" v-focus class="ss" name="message" id="message" rows="3" placeholder="分享快乐的源泉">
        </textarea>
    </div>
...
<script>
  export default {
    ...
    methods: {
        ...
        if(this.content.length<1){
          ...
          for(var i=0;i<this.content.length;i++){
           rows.push(new Promise((resolve,reject)=>{
              var obj={uid:this.uid,content:this.content[i]};
              this.axios.post(this.url1,this.qs.stringify(obj))
              .then(res=>{
              console.log(1)
      ...
      fb(){
        if(this.msg=="" && this.content.length<1){
      ...
      if(!file.length){
        ...
        for(var i=0;i<file.length;i++){
          this.content.push(file[i].content);
          console.log(this.content)
          filesize+=file[i].file.size
        }
      }
      console.log(this.content)
...
<style scoped>
  ...
  .ss{
    resize: none; width: 100%; height: 80px; margin-top: 20px;
    border:0;
    outline: 0;
  }
